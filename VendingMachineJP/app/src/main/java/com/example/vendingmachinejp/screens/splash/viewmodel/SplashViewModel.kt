...
class SplashViewModel @Inject constructor(private val repository: HomeRepository): ViewModel(){
    ...
    var brandInfoData by mutableStateOf<BrandInfoModel?>(null)
        private set

    var brandErrorMessage by mutableStateOf<String?>(null)
        private set

    var brandLoading by mutableStateOf(false)
        private set
    ...
    fun getBrandInfo(
        ...
        viewModelScope.launch {
            isLoading = true
            errorMessage = null
            when (val result = repository.getSupportInfo(apiKey, branchId, organizationId, tenantId)) {
                is DataState.Success -> brandInfoData = result.data
                is DataState.Error -> brandErrorMessage = result.message
                is DataState.Loading<*> -> brandLoading
            }
            isLoading = false
        }
    }
